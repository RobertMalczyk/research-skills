...
Three composable skills for serious research work — investigating hard problems, pivoting to new directions when an approach fails, and producing auditable design documents before committing to code.
...
|---|---|---|
| `hypothesis-verificator` | Why isn't this working? | Hypothesis register + findings |
...
Use when: you have a hard problem, multiple possible explanations, and need to know which (if any) is right.
...
hypothesis-verificator
       │
       │ findings: F1, F2, F3, follow-on questions
       ▼
project-pivot
       │
       │ (Step 8.5 invokes architecture-proposal as a hard gate)
       │
...
`project-pivot` will not write any files until `architecture-proposal` has produced an explicitly approved design document. This is enforced at three levels in the skill: operating principle, mandatory step, and Step 9 precondition check.
